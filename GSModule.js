class GSModule {
	...
	SendData(key, attr, data, callback) {
		...
		this._gamesparks.sendWithData('LogEventRequest', output, response => {
	...
	registerUser(callback) {
		'use strict';
		this._gamesparks.registrationRequest(
			FBINSTANT_INFO.playerInfo.displayName,
			FBINSTANT_INFO.playerInfo.id,
			FBINSTANT_INFO.playerInfo.id,
			response => {
				console.log('[GSModule] Successfully registered');
				if (this.debug === true) console.log(JSON.stringify(response));
				if (callback) callback();
			}
		);
	}
	...
	signIn(callback) {
		...
		this._gamesparks.authenticationRequest(
			FBINSTANT_INFO.playerInfo.id,
			FBINSTANT_INFO.playerInfo.id,
			response => {
				if (this.debug === true) console.log(JSON.stringify(response));
